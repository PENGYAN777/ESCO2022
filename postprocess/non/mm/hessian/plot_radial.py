...
gd= pd.read_csv("../../../../nonideal/mm/hessian/d/radial.csv", ",", skiprows=0)
...
axes.plot(gd.iloc[:,7] , gd.iloc[:,0]/refer.iloc[0,0]*12.9 , 'r', lw=lwh,  label="$H(\\rho)$")
...
fig1.savefig("refer_mm_hes_rho.pdf")
...
# axes.set_ylim(0.2,1)
#axes.legend(loc=2) # 2 means left top
fig2.savefig("refer_mm_hes_m.pdf")

fig3 = plt.figure( dpi=300)
lwh = 2
axes = fig3.add_axes([0.15, 0.15, 0.7, 0.7]) #size of figure
axes.plot(refer.iloc[:,7] , refer.iloc[:,3]/refer.iloc[0,3] , 'k', lw=lwh, label="reference")
axes.plot(gd.iloc[:,7] , gd.iloc[:,3]/refer.iloc[0,3]*300, 'r', lw=lwh,  label="$H(\\rho)$")
...
# axes.set_ylim(0.2,1)
#axes.legend(loc=2) # 2 means left top
fig3.savefig("refer_mm_hes_t.pdf")
